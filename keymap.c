...
enum custom_keycodes {
	MACRO00 = SAFE_RANGE, //password IKM PC
	MACRO01, // ./administrator
	MACRO02, //personnummer
	MACRO03, //izwan.peng@gmail.com
	MACRO04, //ANSYS take picture
	MACRO05, //ACROBAT accept comment
	MACRO06, //EXCEL insert table
	MACRO07, //EXCEL insert figure
	MACRO08, //EXCEL insert docx reference
...
bool process_record_user(uint16_t keycode, keyrecord_t *record) {
	...
	case MACRO00: // IKM PC password
		if (record->event.pressed) {
		   SEND_STRING(SS_TAP(X_RGHT) SS_DELAY(500) SS_TAP(X_RGHT) "AdamAva2122" SS_TAP(X_ENT)); 
		}
		break;
	
	case MACRO01: // admin password and login
		if (record->event.pressed) {
		   SEND_STRING(SS_TAP(X_RGHT) ".\\administrator" SS_TAP(X_TAB) "2Ja7AV52" SS_TAP(X_ENT)); 
		}
		break;
	
	case MACRO02: // personnummer
		if (record->event.pressed) {
		   SEND_STRING("01099323724" SS_TAP(X_ENT)); 
		}
		break;	
	
	case MACRO03: // email personal
		if (record->event.pressed) {
		   SEND_STRING("izwan.peng@gmail.com"); 
		}
		break;	
